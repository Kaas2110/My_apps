...
print(dirname)
Form, Window = uic.loadUiType(dirname+"\\tracker.ui")  # Для того, чтобы привязать к планировщику указываем полный путь до файла
...
def save_to_file():
    ...
    data_to_save = {"start": start_date, "end": calc_date, "desc": description}   # создаем словарь
    file1 = open(dirname+"\\config.txt", "wb")
    pickle.dump(data_to_save, file1)    # сохраняем в файл словарь
    file1.close()
    """ 
    привязываем наше приложение к планировщику задач Windows 
    """
    task = """schtasks /create /tr "python """+os.path.realpath(__file__)+"""" /tn "Трекер события" /sc MINUTE /mo 120 /ed 31/12/2021 /F"""
...
def read_from_file():
    ...
    try:
        ...
        percent = int(delta_days_left * 100 / days_total) # вычисляем в процентном соотношении время до начала события
        form.progressBar.setProperty("value", percent)
    except:
        print("файл не существует, он будет создан после выбора даты события")
...
def on_click_calendar():    # создаем функцию для работы с календарем
